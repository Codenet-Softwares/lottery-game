...
const LiveMarketStats = ({ marketId, backButton }) => {
  ...
  return (
   ...
   <div className="d-flex justify-content-between align-items-center mb-4">
     ...
     <div className="d-flex justify-content-start" style={{ position: 'absolute', left: '40px' }}>
       {backButton}
     </div>
   ...
   <div className="d-flex align-items-center mb-3">
     <input
       ...
       onFocus={(e) => (e.target.style.boxShadow = '0 4px 15px rgba(0, 0, 0, 0.2)')}
   ...
   <div className="table-responsive">
     <table className="table table-striped table-bordered table-hover shadow m-0">
       <thead className="bg-primary text-white">
         <tr>
           <th style={{ textAlign: 'center', border: 'none' }}>Serial Number</th>
           <th style={{ width: '29.3%', textAlign: 'center', border: 'none' }}>Username</th>
           <th style={{ width: '30%', textAlign: 'center', border: 'none' }}>Total Amount</th>
           <th style={{ width: '31.1%', textAlign: 'center', border: 'none' }}>Actions</th>
         </tr>
       </thead>
     </table>
         ...
         <tbody>
           {stats.map((user, idx) => (
             <tr key={idx} style={{ border: 'none' }}>
               ...
               <td className="fw-bold text-secondary" style={{ width: '30%', textAlign: 'center' }}>
                 {user.userName}
               </td>
               <td className="fw-bold text-success" style={{ width: '30%', textAlign: 'center' }}>
                 ₹{user.amount}
               </td>
               <td style={{ width: '30%', textAlign: 'center' }}>
                 <button className="btn btn-info" onClick={() => handleShowTickets(user.details)}>
